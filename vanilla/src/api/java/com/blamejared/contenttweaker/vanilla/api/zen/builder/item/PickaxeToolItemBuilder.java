...
public final class PickaxeToolItemBuilder extends ToolItemBuilder<PickaxeToolItemBuilder> {
    private static final class PickaxeHandles {
        static final MethodHandle INIT = Handles.trusted().linkConstructor(PickaxeItem.class, Tier.class, int.class, float.class, Item.Properties.class);

        private PickaxeHandles() {}
    }
    ...
    private Item build(final ToolData data, final Supplier<Item.Properties> properties) {
        return Handles.invoke(
                () -> (PickaxeItem) PickaxeHandles.INIT.invokeExact(data.tier().get(), (int) data.baseAttackDamage(), data.attackSpeed(), properties.get())
        );
    }
}

...
public final class ShovelToolItemBuilder extends ToolItemBuilder<ShovelToolItemBuilder> {
    private static final class ShovelHandles {
        static final MethodHandle INIT = Handles.trusted().linkConstructor(ShovelItem.class, Tier.class, float.class, float.class, Item.Properties.class);

        private ShovelHandles() {}
    }
    ...
    private Item build(final ToolData data, final Supplier<Item.Properties> properties) {
        return Handles.invoke(
                () -> (ShovelItem) ShovelHandles.INIT.invokeExact(data.tier().get(), data.baseAttackDamage(), data.attackSpeed(), properties.get())
        );
    }
}
